...
export const ProjectCard = ({ headerText, bodyText, link, image, experiment = false }: ProjectCardProps) => {
	...
	return experiment ? (
		<div className={css.containerSmall}>
			<img src={image} alt={`${headerText}-banner`} className={css.imageSmall} />
			<div>
				<h2 className={css.headerTextSmall}>{headerText}</h2>
				<p className={css.bodyTextSmall}>{bodyText}</p>
			</div>
			{link ? (
				<p className={css.linkSmall} onClick={handleLinkClick}>
					SEE MORE
				</p>
			) : (
				<></>
		...
		<div className={css.container}>
			<div className={css.body}>
				...
				{link ? (
					<p className={css.link} onClick={handleLinkClick}>
						SEE MORE
					</p>
				) : (
					<></>
			...
			<img src={image} alt={`${headerText}-banner`} className={css.image} />
		</div>
	);
};
